...
### Publication LLM Summaries (`_summaries/`)

Each file in `_summaries/` is named `<citekey>.md` using the underscore form that matches the QMD filename (e.g. `amonkar_compound_2023.md`, not `amonkar_compound:2023`).
The file contains only the summary text (plain Markdown, no frontmatter).
A Pandoc Lua filter (`_filters/pub-summary.lua`) injects this content into the corresponding pub page at render time.
The filter is a no-op if no sidecar exists.

To generate summaries: ensure Zotero desktop is running and `ANTHROPIC_API_KEY` is set, then run `uv run scripts/generate-pub-summaries.py`.
The script skips citekeys that already have a sidecar file, so it is safe to re-run for new papers only.
